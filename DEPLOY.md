...
AMB_MIGRATION_ENABLED=1    # открыть «Я действующий амбассадор» (перенос из WhatsApp).
                           # Пока переменной НЕТ — раздел закрыт: кнопка скрыта,
                           # действие и deep-link ?start=migrate отвечают заглушкой.
                           # Включать только после этапа 2 (код миграции + ревью заявок).
